...
namespace Project
{
    class Class_Islemler
    {
        ...
        public bool Kullanici_Kontrol(string KullaniciAd, string Parola) //Kullanıcı bilgilerinin kontrolü
        {
            try
            {
                baglanti.Open();
        ...
        public bool Kontrol(string tablo, string kod) //Herhangibir tablodaki aranan kayıtları listeleme
        {
            try
            {
                baglanti.Open();
        ...
        public DataSet Kayitlar(string tablo, string aranan) //Herhangibir tablodaki aranan kayıtları listeleme
        {
            try
            {
                baglanti.Open();
                string sorgu = "select * from " + tablo + " Where "
                    + " code LIKE '%" + aranan + "%' or"
                    + " fall LIKE '%" + aranan + "%' or"
                    + " spring LIKE '%" + aranan + "%' or"
                    + " theory LIKE '%" + aranan + "%' or"
                    + " lab LIKE '%" + aranan + "%' or"
                    + " ects LIKE '%" + aranan + "%'";
                SqlCommand komut = new SqlCommand(sorgu, baglanti);
                SqlDataAdapter da = new SqlDataAdapter(komut);
        ...
        public void CBSec(ComboBox box, string tableName, int Id)//Herhangi bir Combobox nesnesinin verisetinden index seçme
        {
            List<string> data = Donustur(Kayitlar(tableName), 0);

            for (int i = 0; i < data.Count; i++)
                if (Id == Convert.ToInt32(data[i]))
                    box.SelectedIndex = i;
        }
        public void CBSec(ComboBox box, List<string> data, int Id)//Herhangi bir Combobox nesnesinin verisetinden index seçme
        {
            for (int i = 0; i < data.Count; i++)
                if (Id == Convert.ToInt32(data[i]))
                    box.SelectedIndex = i;
        ...
        public int GetirId(string tableName, int index) //Herhangibir tablodan index noya göre id bilgisini getirme
        {
            try
            {
                DataSet data = Kayitlar(tableName);
                List<string> list = Donustur(data, 0);
