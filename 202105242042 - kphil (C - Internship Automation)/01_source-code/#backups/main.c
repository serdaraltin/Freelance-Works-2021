...
void DosyaOkuStaj() { 
  ...
  while (!feof(dosya)) {
    ...
    fscanf(dosya, "%s %s %s %s %s\n", (*stajlar + sstaj)->tur,
           (*stajlar + sstaj)->ogrno, (*stajlar + sstaj)->frmvergiNo,
           (*stajlar + sstaj)->baslangicTarih, (*stajlar + sstaj)->bitisTarih);
    sstaj++;
  }
  fclose(dosya);
}
...
void DosyaYazStaj() {
  ...
  for (i = 0; i < sstaj; i++) {
    fprintf(dosya, "%s %s %s %s %s\n", (*stajlar + i)->tur,
            (*stajlar + i)->ogrno, (*stajlar + i)->frmvergiNo,
            (*stajlar + i)->baslangicTarih, (*stajlar + i)->bitisTarih);
  }
  fclose(dosya);
}
...
void StajEkle() {
  ...
  int donanimStajAdet = StajMiktar(no, "donanim");
  int yazilimStajAdet = StajMiktar(no, "yazilim");
  ...
  printf("Tur [donanim/yazilim] : ");
  char tur[20];
  scanf("%s", tur);

  if (!(strcmp(tur,"donanim") || strcmp(tur,"yazilim"))) {
    printf("Hatali secim !\n");
    return;
  }
  if (strcmp(tur,"donanim")) {
    if (donanimStajAdet >= 4 && yazilimStajAdet < 2) {
      printf("Uygun tur secilmedi !\n");
      return;
    }
  }
  if (strcmp(tur,"yazilim")) {
    if (donanimStajAdet < 4 && yazilimStajAdet >= 2) {
      printf("Uygun tur secilmedi !\n");
      return;
    }
...
void TamamlanmamisStajlar() {
  ...
  int i;
  for (i = 0; i < sstaj; i++) {
    int donanimStaj = StajMiktar((*stajlar + i)->ogrno, "donanim");
    int yazilimStaj = StajMiktar((*stajlar + i)->ogrno,"yazilim");

    if (donanimStaj <= 4 || yazilimStaj <= 2 ||
        donanimStaj + yazilimStaj < 12) {
      int ogrIndex = OgrenciKontrol((*stajlar + i)->ogrno);
      printf("%s",(*stajlar + i)->ogrno);
      YazdirOgrenci((*ogrenciler + ogrIndex));
