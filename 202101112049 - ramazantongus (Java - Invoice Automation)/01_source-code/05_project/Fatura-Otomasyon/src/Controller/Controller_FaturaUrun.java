...
public class Controller_FaturaUrun extends Controller_Main {

    public Controller_FaturaUrun() {
        super(new Model_FaturaUrun());
    }

    @Override
    public boolean Kontrol(Model_Main model) {
        Model_FaturaUrun model2 = (Model_FaturaUrun) model;
        ...
        try {
            ...
            Logger.getLogger(Controller_FaturaUrun.class.getName()).log(Level.SEVERE, null, ex);
            return false;
        }
    }
    ...
    public ArrayList<Model_Main> Listele(JTable Tablo) {
        try {
            ...
            while (resultSet.next()) {
                Model_FaturaUrun kayit = new Model_FaturaUrun(resultSet);
    ...
    public ArrayList<Model_Main> Ara(JTable Tablo, String aranan) {
        try {
            connection = new Controller_Veritabani().Baglanti();
            resultSet = statement.executeQuery(new Model_FaturaUrun().getSearch(aranan));
    ...
    @Override
    public Model_FaturaUrun Getir(Model_Main model) {
        Model_FaturaUrun model2 = (Model_FaturaUrun) model;
        model2.setTabloAd(super.getTabloAd());
        try {
            connection = new Controller_Veritabani().Baglanti();
            ...
            while (resultSet.next()) {
                model2 = new Model_FaturaUrun(resultSet);
                resultSet.close();
                connection.close();
                return model2;
            }
    ...
    public void Guncelle(int Id) {
        new View.View_FaturaUrun(Id).setVisible(true);
    }

    @Override
    public void Ac() {
        new View.View_FaturaUrun().setVisible(true);
    }
