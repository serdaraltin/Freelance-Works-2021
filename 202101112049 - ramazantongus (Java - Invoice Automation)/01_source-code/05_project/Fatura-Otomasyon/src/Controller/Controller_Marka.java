...
public class Controller_Marka extends Controller_Main {

    public Controller_Marka() {
        super(new Model_Marka());
    }

    @Override
    public boolean Kontrol(Model_Main model) {
        Model_Marka model2 = (Model_Marka) model;
        ...
        try {
            ...
            Logger.getLogger(Controller_Marka.class.getName()).log(Level.SEVERE, null, ex);
            return false;
        }
    }
    ...
    public ArrayList<Model_Main> Listele(JTable Tablo) {
        try {
            ...
            while (resultSet.next()) {
                Model_Marka kayit = new Model_Marka(resultSet);
    ...
    public ArrayList<Model_Main> Ara(JTable Tablo, String aranan) {
        try {
            connection = new Controller_Veritabani().Baglanti();
            resultSet = statement.executeQuery(new Model_Marka().getSearch(aranan));
    ...
    @Override
    public Model_Marka Getir(Model_Main model) {
        Model_Marka model2 = (Model_Marka) model;
        model2.setTabloAd(super.getTabloAd());
        try {
            connection = new Controller_Veritabani().Baglanti();
            ...
            while (resultSet.next()) {
                model2 = new Model_Marka(resultSet);
                resultSet.close();
                connection.close();
                return model2;
            }
    ...
    public void Guncelle(int Id) {
        new View.View_Marka(Id).setVisible(true);
    }

    @Override
    public void Ac() {
        new View.View_Marka().setVisible(true);
    }
