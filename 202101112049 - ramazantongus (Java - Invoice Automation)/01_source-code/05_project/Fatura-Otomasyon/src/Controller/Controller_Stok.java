...
public class Controller_Stok extends Controller_Main {

    public Controller_Stok() {
        super(new Model_Stok());
    }

    @Override
    public boolean Kontrol(Model_Main model) {
        Model_Stok model2 = (Model_Stok) model;
        ...
        try {
            ...
            Logger.getLogger(Controller_Stok.class.getName()).log(Level.SEVERE, null, ex);
            return false;
        }
    }
    ...
    public ArrayList<Model_Main> Listele(JTable Tablo) {
        try {
            ...
            while (resultSet.next()) {
                Model_Stok kayit = new Model_Stok(resultSet);
    ...
    public ArrayList<Model_Main> Ara(JTable Tablo, String aranan) {
        try {
            connection = new Controller_Veritabani().Baglanti();
            resultSet = statement.executeQuery(new Model_Stok().getSearch(aranan));
    ...
    @Override
    public Model_Stok Getir(Model_Main model) {
        Model_Stok model2 = (Model_Stok) model;
        model2.setTabloAd(super.getTabloAd());
        try {
            connection = new Controller_Veritabani().Baglanti();
            ...
            while (resultSet.next()) {
                model2 = new Model_Stok(resultSet);
                resultSet.close();
                connection.close();
                return model2;
            }
    ...
    public void Guncelle(int Id) {
        new View.View_Stok(Id).setVisible(true);
    }

    @Override
    public void Ac() {
        new View.View_Stok().setVisible(true);
    }
